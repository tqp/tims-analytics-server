...
import com.timsanalytics.apps.realityTracker.competition.services.CompetitionBestPickService;
...
public class CompetitionBestPickController {
    private final CompetitionBestPickService bestPickService;

    @Autowired
    public CompetitionBestPickController(CompetitionBestPickService bestPickService) {
        this.bestPickService = bestPickService;
    }

    @GetMapping(value = "/{a_teamKey}/{b_userKey}/{c_roundNumber}", produces = "application/json")
    @Operation(summary = "getBestPicksByTeamUserRound", description = "Get Best Picks by Team, User, Round", security = @SecurityRequirement(name = "bearerAuth"))
    public List<CompetitionBestPick> getBestPicksByTeamUserRound(@Parameter(description = "Team Key", example = "key_team1") @PathVariable("a_teamKey") String a_teamKey,
